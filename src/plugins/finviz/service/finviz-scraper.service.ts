...
export class FinVizScraperService implements IScraperServicePlugin {
  ...
  async save(items: FinVizModel[]): Promise<number | undefined> {
    const finVizModel = getModelForClass(FinVizModel);
    await Promise.all(
      items.map(async (item) => {
        await finVizModel.create(item);
      })
    );
   ...
   */
  async scrape(symbol: string): Promise<FinVizModel | undefined> {
